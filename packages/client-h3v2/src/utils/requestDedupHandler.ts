import { type EventHandler, type EventHandlerRequest, defineHandler, getCookie } from 'h3';
...
export const defineDeduplicatedEventHandler = <T extends EventHandlerRequest, D>(
      ...
      lockKey?: string
        ...
        return defineHandler(async (event) => {
            ...
            if (baseKey !== 'anon') {
                const requestMethod = event.req.method || 'GET';
                const requestPath = (event.url.pathname || event.req?.url || event.runtime?.node?.req?.url || 'request').split('?')[0];
